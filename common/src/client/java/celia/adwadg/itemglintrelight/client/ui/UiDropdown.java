...
public final class UiDropdown implements UiComponent {
    ...
    public void render(GuiGraphics graphics, Font font, int mouseX, int mouseY) {
        ...
        graphics.fill(x, fieldY, x + width, fieldY + 22, UiMath.mix(UiPalette.DIVIDER, UiPalette.BRIGHT_BLUE, fieldHover));
        graphics.fill(x + 1, fieldY + 1, x + width - 1, fieldY + 21, UiMath.mix(UiPalette.SURFACE, UiPalette.SURFACE_HOVER, fieldHover));
        String selectedText = options.get(selected);
        SmoothTextRenderer.draw(graphics, font, selectedText, x + 8,
                fieldY + (22 - SmoothTextRenderer.height(selectedText, 0.82F, UiPalette.TEXT)) / 2.0F, 0.82F, UiPalette.TEXT);
        String expandSymbol = expanded ? "-" : "+";
        SmoothTextRenderer.draw(graphics, font, expandSymbol, x + width - 14,
                fieldY + (22 - SmoothTextRenderer.height(expandSymbol, 0.82F, UiPalette.BRIGHT_BLUE)) / 2.0F, 0.82F, UiPalette.BRIGHT_BLUE);
        int visibleHeight = Math.round(options.size() * 22.0F * expansion);
        if (visibleHeight > 0) {
            graphics.fill(x, fieldY + 23, x + width, fieldY + 23 + visibleHeight, UiPalette.BRIGHT_BLUE);
            graphics.fill(x + 1, fieldY + 24, x + width - 1, fieldY + 22 + visibleHeight, UiPalette.DEEP_BLUE_FADE);
            if (!expanded) {
                return;
            }
            for (int index = 0; index < options.size(); index++) {
                ...
                graphics.fill(x + 1, optionY, x + width - 1, optionY + 22, UiMath.mix(UiPalette.DIVIDER, UiPalette.BRIGHT_BLUE, optionHovers[index]));
                graphics.fill(x + 2, optionY + 1, x + width - 2, optionY + 21, UiMath.mix(UiPalette.DEEP_BLUE_FADE, UiPalette.SURFACE_HOVER, optionHovers[index]));
                ...
                if (reveal >= 0.2F) {
                    SmoothTextRenderer.draw(graphics, font, option, x + 8,
                            ...
                            0.78F, index == selected ? UiPalette.LIGHT_GREEN : UiPalette.TEXT);
                }
            }
        }
    }
